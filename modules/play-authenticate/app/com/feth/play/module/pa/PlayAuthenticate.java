...
public abstract class PlayAuthenticate {
	...
	public abstract static class Resolver {
		 ...
		 * If you return null here, the user will get redirected to the URL of the setting afterAuthFallback
		 ...
		 * com.feth.play.module.pa.controllers.AuthenticateController.authenticate(String)
		 * however you might provide your own authentication implementation if
		 * you want to, and point it there
		 * 
		 * @param provider
		 *      The provider ID matching one of your registered providers in play.plugins
		 */
		public abstract Call auth(final String provider);

		/**
		 * If you set the accountAutoMerge setting to true, you might returnnull for this.
		 */
		 ...
		 * If you return null here, the user will get redirected to the URL of the setting afterLogoutFallback
		 * You can use this to redirect to an external URL for example.
		 */
	...
	public static String storeOriginalUrl(final Http.Context context) {
		...
		if (context.request().method().equals("GET") && !context.request().path().equals(loginUrl)) {
			Logger.debug("Path where we are coming from (" + context.request().uri()
	...
	public static boolean isLoggedIn(final Session session) {
		...
		ret &= AuthProvider.Registry.hasProvider(session.get(PROVIDER_KEY)); // this  provider is active
		if (session.containsKey(EXPIRES_KEY)) {
			// expiration is set
			final long expires = getExpiration(session);
			if (expires != AuthUser.NO_EXPIRATION) {
				ret &= (new Date()).getTime() < expires; // and the session expires after now
			}
		}
		return ret;
	}
	...
	public static Result merge(final Context context, final boolean merge) {
		...
		if (merge) {
			...
			// User declined merge, so log out the old user, and log out with the new one
			loginUser = mergeUser;
		}
	...
	public static Result handleAuthentication(final String provider,
		...
		try {
			...
			if (o instanceof String) {
                return Controller.notFound((String) o);
				...
				// are not logged in (does NOT check expiration)
				...
				// deactivated/deleted, so this is a signup, not a link
				if (isLoggedIn) {
					...
					if (!isLoggedIn) {
						...
						// been deleted/deactivated so kill the session
						logout(session);
						oldUser = null;
					}
				}
				...
				if (isLinked && !isLoggedIn) {
					...
					if (isAccountMergeEnabled()
						...
						// account merge is enabled and the currently logged in user and the one to log in
						...
						if (isAccountAutoMerge()) {
							...
							// Account auto merging is disabled - forward user to merge request page
							...
							return Controller.redirect(c);  // TODO handle merge to account
						}
					} else {
						// the currently logged in user and the new login belong to the same local user,
						// or Account merge is disabled, so just change the login to the new user
						loginUser = newUser;
					}
					...
					if (isAccountAutoLink()) {
						...
						// Account auto linking is disabled - forward user to link suggestion page
						...
						return Controller.redirect(c); // TODO handle link to account
					}
				}
				return loginAndRedirect(context, loginUser);
			...
			if (c != null) {
				return Controller.redirect(c); // TODO beware of this redirect!!
			} else {
